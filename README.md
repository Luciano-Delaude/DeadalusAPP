...
1) Push this repo to GitHub (include `streamlit_app.py`, `rubric_validator.py`, `requirements.txt`, `sample_data/`; keep real secrets out of the repo).
2) In Streamlit Community Cloud, create a new app pointing to your repo/branch and set the entrypoint to `streamlit_app.py`.
3) Add secrets in the app settings (not in the repo):
   - `OPENAI_API_KEY="your-key"`
   - `OPENAI_BASE_URL="https://api.openai.com/v1"` (or your custom gateway)
   - `ENABLE_DRY_RUN=true` (optional, to show the dry-run button)
4) Deploy. The app will install from `requirements.txt` and use your secrets. Choose your model in the UI.
### Output
